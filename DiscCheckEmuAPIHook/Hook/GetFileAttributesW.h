...
static DWORD(WINAPI* OGGetFileAttributesW)(
	LPCWSTR lpFileName) = GetFileAttributesW;

DWORD WINAPI HookedGetFileAttributesW(LPCWSTR lpFileName)
{
	SPDLOG_INFO("---> GetFileAttributesW({0})", string_utils::widestringToString(lpFileName));
	...
	SPDLOG_INFO("<--- GetFileAttributesW({0})", string_utils::widestringToString(LPCWSTR(newFileName.c_str())));

	return OGGetFileAttributesW(LPCWSTR(newFileName.c_str()));
}